...
function State() {
    ...
    //Get Data from API 
    async function geTableCellata(name, status,from_date,to_date,country_id) {
      ...
      const handleFilterFormSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const { name, status, from_date, to_date,country_id } = filterFormData; // Destructure from_date and to_date from filterFormData
        await geTableCellata(name, status, from_date, to_date,country_id);
      };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const endpoint = `${BASE_URL}/api/state/state/add`;
        let em = [];
    ...
    const deleteItem = async () => {
        setLoading(true);
        const endpoint = `${BASE_URL}/api/state/state/${deletedItemId}`;
        let em = [];
        try {
            const data = {
                id: deletedItemId,
        ...
        <>
            ...
            <Container>
                <Box className="breadcrumb">
                    <Breadcrumb routeSegments={[{ name: 'State', path: '/master/State ' },
                    { name: 'Form' }]} />
                </Box>
                {
  ...
  />
              ...
              <Grid item xs={12} md={3}>
                  <Autocomplete
                    options={Countries}
                    getOptionLabel={(country) => country.name}
                    value={Countries.find((country) => country.id === filterFormData.country_id) || null}
                    onChange={(event, newValue) => {
                                ...
                                {tableData?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                    .map((item, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell align="center">{item.country_name}</TableCell>
                                            ...
                                            <TableCell align="right">
                                                ...
                                                <StateEdit editedItem={editedItem} handleClose={handleClose} open={open} />
