...
function ManageCurrencies() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const endpoint = `${BASE_URL}/api/currency/currency/add`;
    let em = [];
  ...
  const deleteItem = async () => {
    setLoading(true);
    const endpoint = `${BASE_URL}/api/currency/currency/${deletedItemId}`;
    let em = [];
    try {
      const data = {
        id: deletedItemId,
    ...
    <>
      ...
      <Container>
        <Box className="breadcrumb">
          <Breadcrumb routeSegments={[{ name: 'Currency', path: '/master/currency ' },
          { name: 'Form' }]} />
        </Box>
        {
        ...
        <Stack spacing={3}>
          <SimpleCard title="Currency Form">
                ...
                {tableData?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                  .map((item, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell align="center">{item.currency_name}</TableCell>
                      ...
                      <TableCell align="right">
                        ...
                        <CurrencyEdit editedItem={editedItem} handleClose={handleClose} open={open} />
