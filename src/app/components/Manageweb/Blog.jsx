...
function Blog() {
  ...
  //Get Data from API 
  //Get Data from API 
  async function geTableCellata(title, status,from_date,to_date,category_id) {
    const endpoint = `${BASE_URL}/api/blog/blog_list`;
  ...
  const handleFilterFormSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const { title, status, from_date, to_date,category_id } = filterFormData; // Destructure from_date and to_date from filterFormData
    await geTableCellata(title, status, from_date, to_date,category_id);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const endpoint = `${BASE_URL}/api/blog/blog/add`;
    let em = [];
  ...
  const deleteItem = async () => {
    setLoading(true);
    const endpoint = `${BASE_URL}/api/blog/blog/${deletedItemId}`;
    let em = [];
    try {
      const data = {
        id: deletedItemId,
  ...
  />
              ...
              <Grid item xs={12} md={3}>
                    ...
                    options={categories}
                    getOptionLabel={(category) => category.name}
                    value={categories.find((category) => category.id === filterFormData.category_id) || null}
                    onChange={(event, newValue) => {
            ...
            <StyledTable id="dataTable" ref={tableRef} sx={{ minWidth: 600 }} aria-label="caption table" >
              ...
              <TableHead>
                ...
                <TableRow>
                  ...
                  <TableCell align="center">Blog Category</TableCell>
        ...
        {item.image ? (
          ...
          <span>No Image Available</span>
                      ...
                      <TableCell align="right">
                        ...
                        <BlogEdit editedItem={editedItem} handleClose={handleClose} open={open} />
