...
function Cancellation({ handleClose, open, editedItem }) {
  ...
  const getUsersDetails = async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/pages/pages/5`,
        {
          method: "GET",
          headers: new Headers({
            "token": token,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const endpoint = `${BASE_URL}/api/pages/pages/update/5`;
    let em = [];
    ...
    <>
       <Container>
        <Box className="breadcrumb">
          <Breadcrumb routeSegments={[{ name: 'Cancellation ', path: '/Manageweb/Cancellation ' },
          { name: 'Form' }]} />
        </Box>
        {
        ...
        <Stack spacing={3}>
          <SimpleCard title="Cancellation Form">
